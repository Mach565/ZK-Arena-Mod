...
local function rerollHQ(unitID)
	...
	for i = 1, 5 do
		...
		if not (unitName == -1) then
			...
			spSetUnitRulesParam(unitID, "HQBuyUnit" .. i, ud, LOS_ACCESS)
			spSetUnitRulesParam(unitID, "HQBuyUnitTier" .. i, tier, LOS_ACCESS)
			spSetUnitRulesParam(unitID, "HQBuyUnitPoolID" .. i, poolID, LOS_ACCESS)
		end
	end
end
...
local function addHQ(unitID)
	spSetUnitRulesParam(unitID, "isHQ", 1, LOS_ACCESS)
	spSetUnitRulesParam(unitID, "HQLevel", 1, LOS_ACCESS)

	for i = 1, 5 do
		spSetUnitRulesParam(unitID, "HQBuyUnit" .. i, -1, LOS_ACCESS)
	end

	for i = 1, 8 do
		spSetUnitRulesParam(unitID, "HQSpawnUnit" .. i, -1, LOS_ACCESS)
		spSetUnitRulesParam(unitID, "HQSpawnUnitCost" .. i, 1, LOS_ACCESS)
		spSetUnitRulesParam(unitID, "HQSpawnUnitCount" .. i, 1, LOS_ACCESS)
	end

	spSetUnitRulesParam(unitID, "HQLevelCost", levelData[1].upgradeCost, LOS_ACCESS)
	spSetUnitRulesParam(unitID, "HQRerollCost", levelData[1].rerollCost, LOS_ACCESS)
...
local function OnHQUpgrade(HQID, HQTeamID)
	local HQLevel = spGetUnitRulesParam(HQID, "HQLevel")
	if HQLevel < 11 then
		...
		spSetUnitRulesParam(HQID, "HQLevel", HQLevel + 1, LOS_ACCESS)
		spSetUnitRulesParam(HQID, "HQLevelCost", levelData[HQLevel + 1].upgradeCost, LOS_ACCESS)
		spSetUnitRulesParam(HQID, "HQRerollCost", levelData[HQLevel + 1].rerollCost, LOS_ACCESS)
...
local function OnHQReroll(HQID, HQTeamID)
	local rerollCost = spGetUnitRulesParam(HQID, "HQRerollCost")
...
local function OnHQBuy(HQID, HQTeamID, buyUnitID)
	local ud = spGetUnitRulesParam(HQID, "HQBuyUnit" .. buyUnitID)
	if ud == -1 then
		return
	end
	...
	for i = 1, 8 do
		local udAtI = spGetUnitRulesParam(HQID, "HQSpawnUnit" .. i)
		if udAtI == ud then
			spSetUnitRulesParam(HQID, "HQBuyUnit" .. buyUnitID, -1, LOS_ACCESS)

			local count = spGetUnitRulesParam(HQID, "HQSpawnUnitCount" .. i)
			local metalCost = UnitDefs[ud].metalCost
			spSetUnitRulesParam(HQID, "HQSpawnUnitCount" .. i, count + 1, LOS_ACCESS)
			spSetUnitRulesParam(
				HQID,
				"HQSpawnUnitCost" .. i,
				math.floor((metalCost - (metalCost * 0.15 * count)) * (count + 1)),
				LOS_ACCESS
			)
	...
	for i = 1, 8 do
		local udAtI = spGetUnitRulesParam(HQID, "HQSpawnUnit" .. i)
		local udTier = spGetUnitRulesParam(HQID, "HQBuyUnitTier" .. buyUnitID)
		local udPoolID = spGetUnitRulesParam(HQID, "HQBuyUnitPoolID" .. buyUnitID)
		if udAtI == -1 then
			spSetUnitRulesParam(HQID, "HQBuyUnit" .. buyUnitID, -1, LOS_ACCESS)

			spSetUnitRulesParam(HQID, "HQSpawnUnit" .. i, ud, LOS_ACCESS)
			spSetUnitRulesParam(HQID, "HQSpawnUnitCount" .. i, 1, LOS_ACCESS)
			spSetUnitRulesParam(HQID, "HQSpawnUnitCost" .. i, math.floor(UnitDefs[ud].metalCost), LOS_ACCESS)
			spSetUnitRulesParam(HQID, "HQSpawnUnitTier" .. i, udTier, LOS_ACCESS)
			spSetUnitRulesParam(HQID, "HQSpawnUnitPoolID" .. i, udPoolID, LOS_ACCESS)
			spendMetal(unitCost, HQTeamID)
			return
		end
	end
...
local function OnHQSell(HQID, HQTeamID, sellUnitID)
	local ud = spGetUnitRulesParam(HQID, "HQSpawnUnit" .. sellUnitID)
	if not (ud == -1) then
		Spring.AddTeamResource(HQTeamID, "metal", spGetUnitRulesParam(HQID, "HQSpawnUnitCost" .. sellUnitID))
		spSetUnitRulesParam(HQID, "HQSpawnUnit" .. sellUnitID, -1, LOS_ACCESS)
		spSetUnitRulesParam(HQID, "HQSpawnUnitCost" .. sellUnitID, 1, LOS_ACCESS)

		local udTier = spGetUnitRulesParam(HQID, "HQSpawnUnitTier" .. sellUnitID)
...
local function OnHQSpawn(HQID, HQTeamID, spawnUnitID)
	local udID = spGetUnitRulesParam(HQID, "HQSpawnUnit" .. spawnUnitID)
	if udID == -1 then
		return
	end

	local unitCost = spGetUnitRulesParam(HQID, "HQSpawnUnitCost" .. spawnUnitID)
	if not spendMetal(unitCost, HQTeamID) then
		return
	end

	local unitCount = spGetUnitRulesParam(HQID, "HQSpawnUnitCount" .. spawnUnitID)
...
function gadget:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOptions, cmdTag, synced)
	if spGetUnitRulesParam(unitID, "isHQ") and cmdID < 0 then
		...
		if
			(Spring.IsPosInLos(x, y, z, allyID) or Spring.IsPosInRadar(x, y, z, allyID))
			and spendMetal(metalCost, unitTeam)
		then
